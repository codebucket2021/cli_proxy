...
- **组化配置管理** — 配置按组管理（v2 格式），每组一个 base_url + 多个 key，旧配置自动迁移
- **组内 Key 轮转** — 基于空闲超时的自动轮转策略，空闲超时后切换下一个 key，活跃时保持当前 key（保住缓存）
- **动态切换配置** — 命令行/UI 界面动态切换服务配置组，无需重启终端，上下文保留
- **OAuth 认证支持** — 支持 auth_token、api_key 及 OAuth 三种认证方式
...
> 将 `x.x.x` 替换为实际下载的版本号，例如 `clp-1.12.0-py3-none-any.whl`。
...
```bash
clp list claude             # 列出 Claude 所有配置组
clp list codex              # 列出 Codex 所有配置组
clp active claude 主力       # 激活 Claude 的"主力"配置组
clp active codex dev        # 激活 Codex 的 dev 配置组
```

配置也可以在 Web UI（http://localhost:3300）中快速添加和切换。

> **v1 → v2 迁移说明：** 如果你之前使用的是旧版扁平配置格式，程序会在首次读取时自动迁移到 v2 组格式，无需手动操作。
